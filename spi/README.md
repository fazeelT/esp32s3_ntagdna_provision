...
3. The system will:
   ...
   - Write a random 16-byte secret to File 02 (while Write is still free)
...
| Read file | ✗ | ✓ | ✗ | ✗ |
| Write file | ✗ | ✗ | ✓ | ✗ |
| Change keys | ✗ | ✗ | ✗ | ✓ (must know current key) |
| Change permissions | ✗ | ✗ | ✗ | ✓ |

**Note:** Key 0 (AppMasterKey) controls key management and configuration but does NOT automatically grant file read/write access. File access is governed strictly by the configured access rights.
...
File 02 is configured with **CommMode.Full** — the secret is always encrypted on the air. An RF eavesdropper cannot extract the secret even by capturing the complete NFC exchange.
...
**Requirement:** The stored keys in the database must match the tag's actual keys. If a previous programming was interrupted (e.g., Key 0 changed but Key 1/2 failed), use the Reset function (`T`) to factory-reset the tag first.

## Known Limitations

1. **Re-programming requires current keys** — ChangeKey for keys 1-4 requires the XOR of the new key with the current key. If the stored key in the DB doesn't match the tag's actual key, the change will fail with INTEGRITY_ERROR (0x1E).
...
- **ChangeKey format**: Proper XOR + CRC32 (JAMCRC per NXP AN12196) for changing non-authenticated key slots
...
| "0x7E" LENGTH_ERROR | Command data too long | Check APDU framing |
| "0x1E" INTEGRITY_ERROR on ChangeKey | OldKey mismatch or CRC wrong | DB key doesn't match tag's actual key; use fresh tag |
| "0xAE" AUTH_ERROR | Wrong key or session expired | Re-tap tag to restart session |
| ChangeKey fails on re-program | Previous key change partially succeeded | DB has stale key; factory reset tag with `T` first |
